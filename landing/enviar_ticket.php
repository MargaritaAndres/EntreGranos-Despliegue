...
if (!$archivo || !file_exists($archivo)) {
    ...
    try {
        ...
        $mail->setFrom('termobriss@gmail.com', 'EntreGranos');
         // ➤ Cliente que hizo la compra
        if (!empty($email)) {
            $mail->addAddress($email, $nombre);
        }
        $mail->addAddress('margaritaandreshernandez@gmail.com', "Maggie");

        $mail->Subject = 'Tu comprobante de compra - Termo Briss';
        $mail->Body    = "Hola $nombre,\n\nGracias por tu compra. Adjuntamos tu ticket en PDF.\n\n¡Disfruta el aroma y el sabor de nuestro delicioso café!";
